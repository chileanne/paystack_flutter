...
class _BirthdayWidgetState extends BaseState<BirthdayWidget> {
  ...
  Widget buildChild(BuildContext context) {
    return CustomAlertDialog(
      ...
      child: Container(
        ...
        child: Column(
          children: <Widget>[
            Image.asset('assets/images/dob.png', width: 30.0, package: 'paystack_flutter_sa'),
            _heightBox,
            Text(
              widget.message,
            ...
            _pickedDate == null
                ...
                : WhiteButton(
                    ...
                    child: Row(
                      ...
                      children: <Widget>[Flexible(flex: 4, child: dateItem(_getMonth())), Flexible(flex: 2, child: dateItem(_getDay())), Flexible(flex: 3, child: dateItem(_getYear()))],
                    ),
                  ),
            SizedBox(
              height: _pickedDate == null ? 5.0 : 20.0,
            ),
            _pickedDate == null ? Container() : AccentButton(onPressed: _onAuthorize, text: 'Authorize'),
            Container(
  ...
  void _selectBirthday() async {
    ...
    if (Platform.isIOS) {
      ...
      DateTime? result = await showDatePicker(context: context, selectableDayPredicate: (DateTime val) => val.year > now.year && val.month > now.month && val.day > now.day ? false : true, initialDate: now, firstDate: DateTime(minimumYear), lastDate: now);
  ...
  Widget dateItem(String text) {
    ...
    return Container(
      ...
      decoration: const BoxDecoration(color: Colors.white, borderRadius: BorderRadius.all(Radius.circular(3.0)), border: Border(top: side, right: side, bottom: side, left: side)),
      child: Text(
        text,
        textAlign: TextAlign.center,
        maxLines: 1,
