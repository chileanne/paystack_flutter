...
void main() {
  ...
  group("$CheckoutWidget", () {
    ...
    group("card", () {
      testWidgets("card checkout is displayed for selectable method", (tester) async {
        final checkoutWidget = buildTestWidget(
          CheckoutWidget(
            publicKey: publicKey,
            bankService: MockedBankService(),
      ...
      testWidgets("card checkout is not displayed for bank method", (tester) async {
        var bankService = MockedBankService();

        when(bankService.fetchSupportedBanks()).thenAnswer((_) => Future.value([]));
